...
function renderComplexMap(divContainer, JsonData, action) {
	...
	jQuery2(".attr-osm").hide();
	jQuery2(".attr-stamen").show();
...
function changeTile(name) {
	...
	switch(name) {
	    case "none":
	        ...
	        jQuery2(".attr-osm").hide();
			jQuery2(".attr-stamen").hide();
	        break;
	    case "osm":
	    	layers.removeAt(0);
	        layers.insertAt(0, layer_osm);
	        jQuery2(".attr-osm").show();
			jQuery2(".attr-stamen").hide();
	        break;
	    case "local-tile":
	    	layers.removeAt(0);
...
function loadMapData(){
	...
	jQuery2(".imgCollapse").on('click',function(){
		jQuery2(".float-panel").width("150px");
		jQuery2$(".float-panel").height("36px");
		jQuery2(".float-panel-content").hide();
		jQuery2(".imgCollapse").hide();
		jQuery2(".imgExpand").show();
		jQuery2(".lblDatas").show();
	});		
	
	jQuery2(".imgExpand, .lblDatas").on('click',function(){
		jQuery2(".float-panel").width("auto");
		jQuery2(".float-panel").height("auto");
		jQuery2(".float-panel-content").show();
		jQuery2(".imgCollapse").show();
		jQuery2(".imgExpand").hide();
		$jQuery2(".lblDatas").hide();
	});		
...
function loadTabData(format){
	...
	jQuery2('#table th').on('click',function(){
		if(jQuery2(this).attr("scope") == "col")
			sortTable(jQuery2(this));
	});	
}
var formData;
function loadChartData( format){
	//var features;
	jQuery2("#choice-x").hide();
	jQuery2("#choice-y-type").hide();
	jQuery2("#choice-y").hide();
	color = d3.scaleOrdinal(d3.schemeCategory20);
	formData = format;

	jQuery2("#choice-type-select").attr('disabled','disabled');
	...
	jQuery2("#choice-type-select").removeAttr('disabled');
...
function renderD3Layer(key, display){
	...
	if(display){
		...
		jQuery2(".caption-list").show();
		var captionPanel = jQuery2(".caption-list");
		captionPanel.html("");
		jQuery2.each(properties[key], function(i, v) {
			var label = v;
			try{
...
function analysePropertiesforMap(){
	...
	jQuery2.each(allProperties, function(i, v) {
		etats = [];
		jQuery2.each(featuresArray, function(index, value) {
			if(etats.length > 25) return false;
		    if (jQuery2.inArray(value.properties[v], etats) === -1) {
	...
	var dataPanel = jQuery2(".data-list");
	dataPanel.html("");
...
function analyseProperties(){
	...
	jQuery2.each(allProperties, function(i, v) {
		...
		jQuery2.each(featuresArray, function(index, value) {
			if(etats.length > 25) return false;
		    if (jQuery2.inArray(value[v], etats) === -1) {
		        etats.push(value[v]);
		    }
		});
...
function labelizeArray(value){
	var res = [];
	jQuery2.each(value, function(i, v) {
		try{
...
function renderTextFeature(feature){
	...
	jQuery2.each(allProps, function(i, v) {
		if(feature[v] != null){
			var label = feature[v];
			try{
				label = decodeURIComponent(escape(label));
...
function tabulate(data) {
		...
		jQuery2.each(data, function(i, row) {
...
function selectTypeKey(){
	...
	var key;
	jQuery2.each(Object.keys(properties), function(i, v) {
		if(v.match(pattern)){
			key = v;
			return false;
		}
...
function createBarChart(){
	...
	svg.append("text")      // text label for the x axis
        ...
        .text(jQuery2('#datasetTitle').text() + " - " + jQuery2('.well ul:first li:nth-child(2)').text() + " - " + today.getDate() + "/"+ (today.getMonth()+1) + "/"+ today.getFullYear());
...
function onChangeChart(){
	var select = jQuery2("#choice-chart-select");
	if(select.val() == "bar"){
		createBarChart();
		onChangeType();
		onChangeX();
		...
		createBarChart();
		onChangeType();
		if(jQuery2(".y-list input[name=y-list]:checked").get().length == 0){
			onCheckAll(true);
		} else {
			onCheckY(null);
		}
		...
		jQuery2("#choice-y-type").hide();
		jQuery2("#choice-y").hide();
		jQuery2("#check-y").hide();
		//todo
	} else if(select.val() == "line"){
		createLineChart();
		jQuery2("#choice-type-select").hide();
		jQuery2("#choice-x").hide();
		jQuery2("#choice-x2").show();
		jQuery2("#choice-y-type").hide();
		jQuery2("#choice-y").show();
		jQuery2("#check-y").hide();
		onChangeX2();
	}
}
...
function onChangeType(){
	var select = jQuery2("#choice-type-select");
	if(select.val() == "repartition"){
		if(jQuery2("#choice-chart-select").val() == "stack"){
			jQuery2("#choice-x").show();
			jQuery2("#choice-y-type").hide();
			jQuery2("#choice-y").hide();
			jQuery2("#choice-x2").hide();
			jQuery2("#check-y").show();
		} else {
			jQuery2("#choice-x").show();
			jQuery2("#choice-y-type").show();
			jQuery2("#choice-y").hide();
			jQuery2("#check-y").hide();
			jQuery2("#choice-x2").hide();
		}
...
function onChangeX(){
	//var select = jQuery2("#choice-x-select");
	if(jQuery2("#choice-chart-select").val() == "bar"){
		renderBarChart(jQuery2("#choice-type-select").val(), jQuery2("#choice-x-select").val(), jQuery2("#choice-y-type-select").val(), jQuery2("#choice-y-select").val());
	} else if(jQuery2("#choice-chart-select").val() == "stack"){
		renderStackBarChart(jQuery2("#choice-type-select").val(), jQuery2("#choice-x-select").val(), jQuery2(".y-list input[name=y-list]:checked").map(function() {  return jQuery2( this ).val();  }));
	} else if(jQuery2("#choice-chart-select").val() == "pie"){
		renderPieChart(jQuery2("#choice-type-select").val(), jQuery2("#choice-x-select").val(), jQuery2("#choice-y-type-select").val(), jQuery2("#choice-y-select").val());
	}
	
}
function onChangeX2(){
	renderLineChart(jQuery2("#choice-x2-select").val(), jQuery2("#choice-y-select").val());
}
function onChangeYType(){
	var select = jQuery2("#choice-y-type-select");
	if(select.val() == "count"){
		jQuery2("#choice-y").hide();
...
function onChangeY(){
	//var select = jQuery2("#choice-y-select");
	if(jQuery2("#choice-chart-select").val() == "bar"){
		renderBarChart(jQuery2("#choice-type-select").val(), jQuery2("#choice-x-select").val(), jQuery2("#choice-y-type-select").val(), jQuery2("#choice-y-select").val());
	} else if(jQuery2("#choice-chart-select").val() == "pie"){
		renderPieChart(jQuery2("#choice-type-select").val(), jQuery2("#choice-x-select").val(), jQuery2("#choice-y-type-select").val(), jQuery2("#choice-y-select").val());
	} else if(jQuery2("#choice-chart-select").val() == "line"){
		renderLineChart(jQuery2("#choice-x2-select").val(), jQuery2("#choice-y-select").val());
	}
}
function onCheckAll(checked){
	if(checked){
		jQuery2(".y-list input[name=y-list]").prop("checked", true);
	} else {
		jQuery2(".y-list input[name=y-list]").prop('checked', false);
	}
	renderStackBarChart(jQuery2("#choice-type-select").val(), jQuery2("#choice-x-select").val(), jQuery2(".y-list input[name=y-list]:checked").map(function() {  return jQuery2( this ).val();  }));
}
function onCheckY(e){
	//console.log(e.checked);
	//console.log(jQuery2(".y-list input[name=y-list]:checked").map(function() {  return jQuery2( this ).val();  }));
	renderStackBarChart(jQuery2("#choice-type-select").val(), jQuery2("#choice-x-select").val(), jQuery2(".y-list input[name=y-list]:checked").map(function() {  return jQuery2( this ).val();  }));
}
	...
	for(var key in allProperties) {
		if(allProperties[key][0] != null){
			if(isNumeric(allProperties[key][0])) {
				jQuery2("#choice-y-select").append(jQuery2('<option>', {  value: key, text: key}));
				jQuery2(".y-list").append('<li class="y-item">'+
							'<input type="checkbox" class="choice-cb" name="y-list" value="'+ key 
							+'" onclick="onCheckY(this)">'+
							'<span class="choice-label">'+ key +'</span>'+
						'</li>');
				jQuery2("#choice-x2-select").append(jQuery2('<option>', {  value: key, text: key}));
			} else {
				if(properties.hasOwnProperty(key)){
					jQuery2("#choice-x-select").append(jQuery2('<option>', {  value: key, text: key}));
				}
			}
		} else {
...
function getSum(xkey, value, ykey){
	...
	var count = 0;
	jQuery2.each(filtered, function(i, v) {
	...
	if(type == "repartition"){
		...
		if(ydatas == null) return;
			...
			var data = properties[xdata].map(function(d){
				...
				var res = [];
				res.name = labelize(d);
				jQuery2.each(ydatas, function(i, v) {
...
function createPieChart(){
	...
	/*label = d3.arc()
	 ...
	 svg.append("text")      // text label for the x axis
        ...
        .attr("font-size", "10px")
        .attr("font-weight", "bold")
        .text($('.infos h1').text() + " - " + $('.well ul:first li:nth-child(2)').text());
...
function createLineChart(){
	...
	svg.append("text")      // text label for the x axis
        ...
        .attr("font-size", "10px")
        .attr("font-weight", "bold")
        .text($('#datasetTitle').text() + " - " + $('.well ul:first li:nth-child(2)').text() + " - " + today.getDate() + "/"+ (today.getMonth()+1) + "/"+ today.getFullYear());
...
function renderLineChart(xdata, ydata){
	...
	_data.sort(function(a, b) { return +(getData(b, xdata).toString().replace(",",".")) - +(getData(a, xdata).replace(",",".")); });
